...
class Liquidations(commands.Cog):
    ...
    async def get_df(self):
        ...
        df = await get_json_data("https://fapi.coinglass.com/api/futures/liquidation/chart?symbol=ALL")
        df = pd.DataFrame(df)
        
        # Set correct column names
        df['Shorts'] = df['data'].apply(lambda x: x['buyVolUsd'])
        df['Longs'] = df['data'].apply(lambda x: x['sellVolUsd'])
        df['time'] = df['data'].apply(lambda x: x['createTime'])
        df['price'] = df['data'].apply(lambda x: x['price'])
    ...
    async def post_liquidations(self):
        ...
        ax2.legend(lines + lines2, labels + labels2, loc="upper center", fontsize="x-small", ncol=3)
        ...
        e.set_footer(text="\u200b", icon_url="https://cdn.coinglasscdn.com/static/icon_200.png")
